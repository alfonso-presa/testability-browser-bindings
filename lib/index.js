...
var instrumentBrowser = function (window, config) {
    ...
    var setImmediate = window.setTimeout;
    ...
    function patchFunction(set, clear, filterTime) {
        ...
        window[set] = function () {
            ...
            var ref;
            var callerId = cb.toString();
            var recursive = stack.indexOf(callerId) >= 0;
    ...
    function detectMutations() {
        function startHandler(event) {
            ...
            if (event.target && !event.target._testabilityAnimating) {
                if(!isInfinite(window.getComputedStyle(event.target)) &&
                    !isInfinite(window.getComputedStyle(event.target, ":after")) &&
                    !isInfinite(window.getComputedStyle(event.target, ":before"))) {
