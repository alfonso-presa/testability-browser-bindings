...
var instrumentBrowser = function (window) {
    ...
    function patchFunction(set, clear, filterTime) {
        ...
        window[set] = function () {
            ...
            if (!filterTime || (time < 5000 && !stack[arguments[0]])) {
                stack[arguments[0]] = true;

                arguments[0] = function () {
                    var rtn = cb.apply(window, arguments);
                    delete stack[arguments[0]];
                    endWait(ref);
                    return rtn;
                };
    ...
    //document.addEventListener(events.transitionstart, window.testability.wait.oneMore);
    //document.addEventListener(events.transitionend, endHandler);

    document.addEventListener(events.animationstart, window.testability.wait.oneMore);
    ...
    return {
        restore: function () {
            ...
            document.removeEventListener(events.transitionstart, window.testability.oneMore);
            document.removeEventListener(events.transitionend, endHandler);

            document.removeEventListener(events.animationstart, window.testability.oneMore);
