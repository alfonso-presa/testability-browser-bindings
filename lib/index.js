...
var instrumentBrowser = function (window, config) {
    ...
    function shouldBeSkipped(url, method) {
      var matcher = function(cfg) { return 'method' in cfg ?  method === cfg.method && url.match(cfg.url) : url.match(cfg.url); };
      var res = false;
    ...
    function patchFunction(set, clear, filterTime) {
        ...
        window[set] = function () {
            ...
            if (!filterTime || (time < maxTimeout && stack[cb] !== cb)) {
    ...
    window.XMLHttpRequest.prototype.open = function () {
        if(window.testability && !shouldBeSkipped(arguments[1], arguments[0])) {
            var task;
